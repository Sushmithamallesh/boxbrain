        ...
        messages: [
          {
            ...
            - orderId: Order/confirmation number
            - vendor: Company name
            - totalAmount: Purchase amount (number)
            - currency: Currency code (e.g., USD, EUR, GBP)
            - orderDate: When order was placed (ISO date string)
            - latestStatus: Current order status (one of: ordered, confirmed, packed, shipped, out_for_delivery, delivered)
            ...
            If the email is not order-related or you can't extract meaningful information, return null.
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      for (let i = 0; i < relevantEmailsData.length; i += BATCH_SIZE) {
        ...
        // Filter out null results and add valid order details
        orderDetails.push(...batchResults.filter((result): result is OrderDetails => result !== null));
